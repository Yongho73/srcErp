...
function cf_SetEventListenerPopupMenu(){
    ...
    $('#pymntDeClosAt').unbind('keydown').bind('keydown',function(event) {
        if (event.keyCode == 13)  {
            $('#pymntDeClosAt').focus();
        }
    });
    $('#btnPymntDeClosAtPupupSearch').unbind('click').bind('click', function() {
        fn_SearchPopupPymntDeClosAt();
    });
    
    $('#btnPymntDeClosAtPupupInit').unbind('click').bind('click', function() {
        gf_FormSetValue('searchForm', 'pymntDeClosAt', '', 'text');
    ...
    $('#btnPopupOk').unbind('click').bind('click', function() {
        ...
        fn_SearchPymntDeClosAtOk(selectedId);
    });
    
    dhxGridPopup.attachEvent("onRowDblClicked", function(rId,cInd){
        fn_SearchPymntDeClosAtOk(rId);
    });
}
...
function fn_SearchPopupPymntDeClosAt(pageNum, key){
    ...
    gf_FormSetValue('searchFormPymntDeClosAtPopup', 'selectedPageNum', page, 'text');
    
    var jsonParameter = {
            applcYm : gf_FormGetValue('searchFormPymntDeClosAtPopup', 'applcYm', 'text'),
    ...
    gf_Transaction(key, 'mpsbsc006/searchMpsbsc006closAtList', jsonParameter, 'fn_CallbackAcntCdPopup', false, 'GET');
   ...
   dhxGridPopup.clearAll();
    ...
    dhxGridPopup.forEachRow(function(rowId) {
        ...
        if (closAt == '1' || closAt == '0'){
            dhxGridPopup.cells(rowId,6).setDisabled(true);
        }
        });

};

function fn_SearchPymntDeClosAtOk(rId){

    obj = eval("$pymntDeClosAtInfo");
     if(!gf_IsNull(rId)){  
...
<div class="pop-content">
        ...
        <div class="consearch_div_in" id="searchFormPymntDeClosAtPopup">
            ...
            <div class="consearchbt_div" >
                <ul class="consearchbt_list">
                    <li><a href="#none" id="btnPymntDeClosAtPupupSearch"><span class="glyphicon glyphicon-search mr5"></span><span>조회</span></a></li>
                    <li><a href="#none" id="btnPymntDeClosAtPupupInit"><span class="glyphicon glyphicon-refresh mr5"></span><span>초기화</span></a></li>
                </ul>
            </div>
        </div>
