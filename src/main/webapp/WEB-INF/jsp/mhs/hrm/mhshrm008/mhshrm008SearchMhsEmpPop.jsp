...
function cf_SetComponentsPopupMenu(){
	...
	var dhxGridTreeMenuInfo = [5];	
	dhxGridTreeMenuInfo[0] = gf_MakeDhxGridHeader('사원번호', '150', 'center', 'str', 'ro', false, 'clsfCode', '');	
	dhxGridTreeMenuInfo[1] = gf_MakeDhxGridHeader('성명',   '250','left','na', 'ro', false, 'clsfNm', '');
	dhxGridTreeMenuInfo[2] = gf_MakeDhxGridHeader('부서명',   '250','left','na', 'ro', false, 'clsfNm', '');
	dhxGridTreeMenuInfo[3] = gf_MakeDhxGridHeader('직종',   '250','left','na', 'ro', false, 'clsfNm', '');
	dhxGridTreeMenuInfo[4] = gf_MakeDhxGridHeader('','*','center','str', 'ro', false, '', '');
...
function cf_SetEventListenerPopupMenu(){
	...
	$('#clsfNm').unbind('keydown').bind('keydown',function(event) {
		if (event.keyCode ===13)  {
			fn_SearchPopupDept();
	    }
    });
...
function fn_SearchPopupDept(){    	
	var jsonParameter = {
			clsfNm : $('#searchFormPopupMenu input[name="clsfNm"]').val(),
			useAt     : gf_FormGetValue('searchFormPopupMenu', 'useAt', 'radio')
	};
	var dataSource = gf_NoAsyncTransaction('mhshrm008/searchMhsClsf', jsonParameter, 'GET');
...
<div class="pop-content">	 
	<div>
		...
		<div class="search_box" id="searchFormPopupMenu">
			<table>
			<tr>
				<th><label for="docmT">직급코드 또는 직급명</label>
				</th>
				<td><input type="text" id="clsfNm" name="clsfNm" class="w390 ml5" />
				</td>
