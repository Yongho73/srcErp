...
var defaults = {
  ...
  template: '<div class="yearpicker-container"><div class="yearpicker-header"><div class="yearpicker-prev" data-view="yearpicker-prev">&lsaquo;</div><div class="yearpicker-current" data-view="yearpicker-current">SelectedYear</div><div class="yearpicker-next" data-view="yearpicker-next">&rsaquo;</div></div><div class="yearpicker-body"><ul class="yearpicker-year" data-view="years"></ul></div></div>',
...
var Yearpicker = (function() {
  ...
  Yearpicker.prototype = {
    ...
    bind: function() {
      ...
      if (this.isInput) {
        $this.on(event_focus, $.proxy(this.showView, this));
    	  //$this.on("click", $.proxy(this.showView, this));
      } else {
    ...
    createItem: function(data) {
      ...
      if (items.selected && items.highlighted) {
    	  //return '<${itemTag} class="${items.view} selected highlighted" data-view="${items.view}">${items.text}</${itemTag}>';
    	  return '<' + itemTag + ' class="' + items.view + ' selected highlighted" data-view="' + items.view + '">' + items.text + '</' + itemTag + '>';
      }
      else if (!items.selected && items.highlighted) {
    	  //alert("highlighted");
    	  //return '<${itemTag} class="${items.view} selected highlighted" data-view="${items.view}">${items.text}</${itemTag}>';
    	  return '<' + itemTag + ' class="' + items.view + ' highlighted" data-view="' + items.view + '">' + items.text + '</' + itemTag + '>';
      }
      else if (items.selected && !items.highlighted) {
    	  //alert("selected");
    	  //return '<${itemTag} class="${items.view} selected highlighted" data-view="${items.view}">${items.text}</${itemTag}>';
    	  return '<' + itemTag + ' class="' + items.view + ' selected" data-view="' + items.view + '">' + items.text + '</' + itemTag + '>';
      }
      else{
    	  var classes = [];
	      if (items.selected) {
	      ...
	      //alert(" itemTag ---- : " + itemTag + "  :  " + '${itemTag}' + "  :  " + "${itemTag}");
	      //return `<${itemTag} class="${items.view} ${classes.join(" ")}" data-view="${items.view}">${items.text}</${itemTag}>`;
	      return '<' + itemTag + ' class="' + items.view + '" data-view="' + items.view + '">' + items.text + '</' + itemTag + '>';
      }
    }
  };
