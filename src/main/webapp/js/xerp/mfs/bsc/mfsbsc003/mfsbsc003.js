...
var cf_InitParamMfsbsc003 = function() {
    ...
    gf_ComboCode('divComboDpstKind', 'dpstKind', 'dpstKind', 'sel', 'C012', '' , '', '', 'ordr', 'required','','');   //예금종류  
    
    var dhxCalendarStartDate = new dhtmlXCalendarObject([{input:"estblDeSaveFormMfsbsc003", button:"startDateIcon"},{input:"exprtnDeSaveFormMfsbsc003", button:"startDateIcon"},{input:"trmnatDeSaveFormMfsbsc003", button:"startDateIcon"}]);
...
var cf_SetComponentsMfsbsc003 = function() {
    ...
    dhxGridMfsbsc003HeaderInfo.push(gf_MakeDhxGridHeader('계좌번호', '*', 'left', 'str', 'ro', false, 'acnutNo', '', '')); /* gf_LocaleTrans('default', 'titAcnutNo') */
    dhxGridMfsbsc003HeaderInfo.push(gf_MakeDhxGridHeader('은행', '150', 'center', 'str', 'ro', false, 'bankNm', '', '')); /* gf_LocaleTrans('default', 'titBankNm') */
    dhxGridMfsbsc003HeaderInfo.push(gf_MakeDhxGridHeader('지점', '200', 'center', 'str', 'ro', false, 'estblBhf', '', '')); /* gf_LocaleTrans('default', 'titEstblBhf') */
    //dhxGridMfsbsc003HeaderInfo.push(gf_MakeDhxGridHeader('사용여부', '100', 'center', 'str', 'ro', false, 'useAt', '', ''));
    //dhxGridMfsbsc003HeaderInfo.push(gf_MakeDhxGridHeader('사용여부', '100', 'center', 'str', 'useAtro', false, 'useAt', '', ''));
...
var fn_SearchMfsbsc003 = function(key) {
    var jsonParameter = {
        ...
        useAt : gf_FormGetValue('searchFormMfsbsc003', 'useAt', 'combo')
    };
...
var fn_FindMfsbsc003 = function() {
    ...
    if (!gf_IsNull(acnutNo)) {
        ...
        gf_FormSetValue('saveFormMfsbsc003', 'dpstKind', data.dpstKind, 'combo');
        gf_FormSetValue('saveFormMfsbsc003', 'dpstNm', data.dpstNm, 'text');
        gf_FormSetValue('saveFormMfsbsc003', 'dpstrNm', data.dpstrNm, 'text');
        gf_FormSetValue('saveFormMfsbsc003', 'bankCode', data.bankCode, 'combo');
        ...
        gf_FormSetValue('saveFormMfsbsc003', 'bassBnkbAt', (( data.bassBnkbAt == '1') ? true : false), 'chkbox');
...
var fn_InitInputFormMfsbsc003 = function() {
    ...
    gf_FormSetValue('saveFormMfsbsc003', 'useAt', "1", 'chkbox'); // 
    gf_FormSetValue('saveFormMfsbsc003', 'bplcCode', gBplcCode, 'text'); // 
};
/**
 * 데이터 중복 체크
 */
...
var fn_SaveMfsbsc003 = function() {    
    if($('#saveFormMfsbsc003').validate().form()){
        ...
        var jsonParameter = {
            ...
            dpstKind : gf_FormGetValue('saveFormMfsbsc003', 'dpstKind', 'combo'),
            dpstNm : gf_FormGetValue('saveFormMfsbsc003', 'dpstNm', 'text'),
            dpstrNm : gf_FormGetValue('saveFormMfsbsc003', 'dpstrNm', 'text'),
            bankCode : gf_FormGetValue('saveFormMfsbsc003', 'bankCode', 'combo'),
            //bankNm : gf_FormGetValue('saveFormMfsbsc003', 'bankNm', 'text'),
            bankNm : $("#bankCode option:selected").text(), 
            estblBhf : gf_FormGetValue('saveFormMfsbsc003', 'estblBhf', 'text'),
            estblDe : gf_FormGetValue('saveFormMfsbsc003', 'estblDe', 'text').replaceAll('-',''),
            exprtnDe : gf_FormGetValue('saveFormMfsbsc003', 'exprtnDe', 'text').replaceAll('-',''),
            trmnatDe : gf_FormGetValue('saveFormMfsbsc003', 'trmnatDe', 'text').replaceAll('-',''),
            intrRt : gf_FormGetValue('saveFormMfsbsc003', 'intrRt', 'text'),
            bassBnkbAt : gf_FormGetValue('saveFormMfsbsc003', 'bassBnkbAt', 'chkboxYN'),
            rm : gf_FormGetValue('saveFormMfsbsc003', 'rm', 'text'),
            useAt : gf_FormGetValue('saveFormMfsbsc003', 'useAt', 'chkboxYN')
        };
...
var fn_RemoveMfsbsc003Send = function() {
    ...
    if(dataSource.code === '000') {
        fn_SearchMfsbsc003();
        $("#checkAllMfsbsc003").prop('checked', false) ;  //상단 체크박스 해제    
    } else {
        gf_DivMsgAlert(gf_LocaleTrans('default','msgFail'));
    }
};
