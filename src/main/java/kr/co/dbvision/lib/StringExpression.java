...
public class StringExpression {
	...
	public static String nullConvert(Object src) {
		...
		if (src != null && src instanceof java.sql.Timestamp) {
			return ((Timestamp) src).toString();
		}
		
		if (src != null && src instanceof java.lang.Long) {
			return ((Long) src).toString();
		}
		
		if (src != null && src instanceof java.lang.Integer) {
			return ((Integer) src).toString();
		}
		
		if (src != null && src instanceof Clob) {
			
			Clob tmpStr = (Clob) src;
			try {
		...
		if (null == src || "null".equals(src) || "".equals(src) || " ".equals(src)) {
			return "";
		} else {
	...
	public static String nullConvert(Object src, String init) {
        // if (src != null &&
        // src.getClass().getName().equals("java.math.BigDecimal")) {
        if (src != null && src instanceof java.math.BigDecimal) {
            return ((BigDecimal) src).toString();
	 ...
	 * </pre>
	 */
	public static String nullConvert(String src, String init) {
	 ...
	 * @return
	 */
	public static String delDot(String date) {
	...
	public static String todayText() {
		String day = new java.text.SimpleDateFormat("yyyyMMddHHmmss").format(new java.util.Date());
		return day;
	}

	public static String getYn(String yn) {
		if (yn.equals("1") || yn.toLowerCase().equals("y")) {
			return "Y";
		} else {
			return "N";
		}
	...
	public static String toCamelCase(Object target) {
		StringBuffer buffer = new StringBuffer();
		for (String token : String.valueOf(target).toLowerCase().split("_")) {
			buffer.append(StringUtils.capitalize(token));
		}
		return StringUtils.uncapitalize(buffer.toString());
	}

	public static String toCamelCaseFirstUpperCase(Object target) {
		String response = toCamelCase(target);
		response = String.valueOf(response.charAt(0)).toUpperCase() + response.substring(1);
		return response;
	}
	
	public static String toFirstUpperCase(Object target) {
		String response = String.valueOf(target);
		response = String.valueOf(response.charAt(0)).toUpperCase() + response.substring(1);
		return response;
	}
	
	public static String lpad( String value, int len, String prefix ) { 
		String strResult = ""; 
		StringBuilder sbAddChar = new StringBuilder(); 
		for( int i = value.length(); i < len; i++ ) {  
			sbAddChar.append( prefix ); 
		} 
		strResult = sbAddChar + value;  
		return strResult;		
	}
	
	public static String rpad( String value, int len, String prefix ) { 
		String strResult = ""; 
		StringBuilder sbAddChar = new StringBuilder(); 
		for( int i = value.length(); i < len; i++ ) {  
			sbAddChar.append( prefix ); 
		} 
		strResult = value + sbAddChar;  
		return strResult;		
	}
}
